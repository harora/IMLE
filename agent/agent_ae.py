...
class PointAEAgent(BaseAgent):
    ...
    def forward(self, data,test=False):
        ...
        encoded_input_pts = data['points_encoded'].cuda()
        target_pts = input_pts.clone().detach()

        self.output_pts = self.net(encoded_input_pts)
        if not test:
    ...
    def get_point_cloud(self,data):
        input_pts = data['points'].cuda()
        input_pts = input_pts.transpose(1, 2).detach().cpu().numpy()
        output_pts = self.output_pts.transpose(1, 2).detach().cpu().numpy()
        return input_pts[0],output_pts[0]
