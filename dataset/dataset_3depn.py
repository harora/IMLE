...
from util.fps import farthest_point_sampling,positional_encoding
...
class AEdataset3DEPN(Dataset):
	def __init__(self, phase, data_root, category, n_pts):
		...
		self.n_pts = n_pts
		print('Number of Shapes: ',len(self.shape_names))
	...
	def __getitem__(self, index):
		...
		pc = farthest_point_sampling(pc,k=256)
		...
		pc = pc.transpose(1, 0)
		return {"points": pc, "id": self.shape_names[index],"points_encoded": enc_pc}
...
class GANdataset3DEPN(Dataset):
	...
	def __getitem__(self, index):
		...
		raw_pc = add_noise_pc(raw_pc,self.phase)
