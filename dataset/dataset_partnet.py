...
def get_dataloader_partnet(phase, config):
    ...
    if config.module == "imle_gan" or config.module == "gan":
...
class AEdatasetPartNet(Dataset):
    ...
    def random_rm_all_parts(self, raw_pc, part_labels):
    ...
    def __getitem__(self, index):
        ...
        label_path = os.path.join(PC_MERGED_LABEL_DIR, self.shape_names[index], 'label-merge-level1-10000.txt')
        part_labels = self.read_point_cloud_part_label(label_path)
        pc, n_part_keep = self.random_rm_all_parts(pc, part_labels)
...
class GANdatasetPartNet(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # raw_shape_name = self.shape_names[index]
        return {"raw": raw_pc, "real": real_pc, "raw_id": raw_shape_name, "real_id": raw_shape_name,
